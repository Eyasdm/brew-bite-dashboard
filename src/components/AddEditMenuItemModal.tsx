...
function mapItemToForm(item: MenuItem | null | undefined): MenuItemFormData {
  return {
    name: item?.name ?? "",
    price: item?.price ?? "",
...
interface AddEditMenuItemModalProps {
  ...
  onClose: () => void;
  onSubmit: (data: MenuItemFormData & { price: number }, resetForm: () => void) => void;
  loading: boolean;
}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <ToggleSwitch
                ...
                onChange={(val: boolean) => setForm((p) => ({ ...p, available: val }))}
              />
            </div>
