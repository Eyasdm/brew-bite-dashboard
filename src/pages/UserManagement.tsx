...
function Badge({ tone = "gray", children }: BadgeProps) {
...
function DeleteConfirmModal({ user, onConfirm, onCancel, loading }: DeleteConfirmModalProps) {
...
/* ================= badge mappers ================= */
function getRoleBadge(role: string, t: TFunction): { tone: string; label: string } {
  return role === "admin"
    ? { tone: "purple", label: t("users.roles.admin") }
    : { tone: "blue", label: t("users.roles.staff") };
}

function getStatusBadge(isActive: boolean, t: TFunction): { tone: string; label: string } {
  return isActive
            ...
            <table className="w-full min-w-[900px]">
              <tbody className="divide-y divide-border">
                {filteredUsers.map((u) => {
                  ...
                  const status = getStatusBadge(u.is_active ?? true, t);
